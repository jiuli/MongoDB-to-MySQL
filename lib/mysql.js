...
//	  console.log('each--------',val,key);
    if (_.contains(keys, key)) {
      ...
      if (self.config.sync_fields[key] === 'string') {
        ...
        values.push(val || 0);
      } else if (self.config.sync_fields[key] === 'datetime') {
    	 values.push("'" + moment(val).format('YYYY-MM-DD HH:mm:ss') + "'");
      }
...
MySQL.prototype.update = function (id, item, unset_items, callback) {
  ...
  if (unset_items) {
    unset_items = this.transTo(unset_items);
  }
  var self = this;
  var keys = _.keys(this.config.sync_fields);
  var sets = [];
    ...
    if (_.contains(keys, key)) {
      if (self.config.sync_fields[key] === 'string') {
        ...
        sets.push(key + ' = ' + (val || 0));
    	 ...
    	 sets.push(key + " = '" + moment(val).format('YYYY-MM-DD HH:mm:ss') + "'");
      }
  ...
  var sql;
  if (this.config.sync_fields['id'] === 'int') {
    sql = 'UPDATE ' + this.config.mysql.table + ' SET ' + sets_str + ' WHERE id = ' + id + ';';
  } else {
    sql = 'UPDATE ' + this.config.mysql.table + ' SET ' + sets_str + ' WHERE id = "' + id + '";';
  }
  var conn = self.getConnection();
...
MySQL.prototype.remove = function (id, callback) {
  var sql;
  if (this.config.sync_fields['id'] === 'int') {
    sql = 'DELETE FROM ' + this.config.mysql.table + ' WHERE id = ' + id + ';';
  } else {
    sql = 'DELETE FROM ' + this.config.mysql.table + ' WHERE id = \'' + id + '\';';
  }
...
//  var sql2 = 'DROP TABLE IF EXISTS mongo_to_mysql; ' + 'CREATE TABLE mongo_to_mysql (service varchar(20), timestamp BIGINT) ENGINE INNODB;';
  var sql = 'CREATE TABLE IF NOT EXISTS ' + this.config.mysql.table + ' (' + fields_str + ') ENGINE INNODB;';
  var sql2 = 'CREATE TABLE IF NOT EXISTS mongo_to_mysql (service varchar(20), timestamp BIGINT) ENGINE INNODB;';
...
MySQL.prototype.transTo = function (item) {
	var keys = _.keys(item);
	var field_names = this.config.trans_to.field_names;
	
	_.each(field_names, function(val, key){
//		console.log('trans_to---------',val,key);
		if (_.contains(keys, key)) {
			item[val] = '' + item[key] + '';
			delete item[key];
		}
